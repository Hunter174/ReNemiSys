...
const RED_GOBLIN = preload("res://scenes/goblins/classes/torch/red_goblin.tscn")

# Initialize faction properties
func _ready():
	self.faction_name = "Red Faction"
	self.color = Color.RED
	self.spawn_point = Vector2(-6259, -250)

# Override method to add blue goblins
func add_member(goblin_name):
	var goblin_instance = RED_GOBLIN.instantiate()
	add_child(goblin_instance)

	goblin_instance.name = goblin_name
	goblin_instance.faction_name = self.faction_name
	goblin_instance.add_to_group("red_faction")
